...
import { loadPlan, setPlanItem, approvedItems, executeItem, testRemote, archive, log } from './actions.js';
...
async function dfNow() {
  ...
  // APFS: capacity = used/(used+avail) — free space is shared across container volumes
  return { sizeKB: +f[1], usedKB: +f[2], availKB: +f[3], pct: Math.round((+f[2] / (+f[2] + +f[3])) * 100) };
}
...
async function handleApi(req, res, url, body) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/plan') {
    ...
    return send(200, setPlanItem(id, { ...(status && { status }), ...(action && { action }) }));
  }
