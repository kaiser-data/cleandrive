...
async function dfData() {
  ...
  const [sizeKB, usedKB, availKB] = [line[1], line[2], line[3]].map(Number);
  // APFS: free space is shared across volumes, so capacity = used/(used+avail), not used/size
  return { sizeKB, usedKB, availKB, pct: Math.round((usedKB / (usedKB + availKB)) * 100) };
}
